...
export class EstabelecimentoController {
    ...
    @ApiResponse({
        status: 500,
        description: 'Erro ao criar o estabelecimento',
    })
    async create(
        @Body(ValidationPipe)
    ...
    async findAll(): Promise<Estabelecimento[]> {
        try {
            ...
            throw new HttpException(
                'Erro ao listar todos os estabelecimentos',
                HttpStatus.INTERNAL_SERVER_ERROR,
            );
        }
    }
    ...
    @ApiOperation({
        summary:
            'Buscar estabelecimentos por múltiplos critérios com paginação',
    })
    @ApiResponse({
        status: 200,
        description: 'Lista de estabelecimentos encontrados',
        type: [Estabelecimento],
    })
    @ApiResponse({ status: 401, description: 'Não autorizado' })
    async search(
        @Query() query: SearchEstabelecimentoDto,
    ): Promise<{
        ...
        try {
            ...
            throw new HttpException(
                'Erro ao buscar estabelecimentos com os critérios fornecidos',
                HttpStatus.INTERNAL_SERVER_ERROR,
            );
        }
    }
    ...
    @ApiResponse({
        status: 500,
        description: 'Erro ao buscar todos os estabelecimentos',
    })
    async findAllByUser(@Request() req) {
        try {
            ...
            if (error instanceof HttpException) {
                ...
                throw new HttpException(
                    'Erro ao buscar todos estabelecimentos',
                    HttpStatus.INTERNAL_SERVER_ERROR,
                );
            }
        }
    ...
    @ApiResponse({
        status: 404,
        description: 'Estabelecimento ou quadras não encontradas',
    })
    async findQuadrasByIdkeyEstabelecimento(
        @Param('idkey', ParseIntPipe) idkey: number,
    ): Promise<Quadra[]> {
        try {
            return await this.estabelecimentoService.findQuadrasByIdkeyEstabelecimento(
                idkey,
            );
        } catch (error) {
            if (error instanceof HttpException) {
    ...
    @ApiParam({
        name: 'idkey',
        description: 'ID do estabelecimento a ser removido',
        example: 1,
    })
    @ApiResponse({
        status: 200,
    ...
    @ApiResponse({
        status: 500,
        description: 'Erro ao remover o estabelecimento',
    })
    ...
    @ApiOperation({ summary: 'Buscar estabelecimentos por tipo de esporte' })
    @ApiParam({
        name: 'tipoEsporteId',
        description: 'ID do tipo de esporte para buscar estabelecimentos',
        example: 1,
    })
    @ApiResponse({
        status: 200,
        description: 'Lista de estabelecimentos filtrados pelo tipo de esporte',
        type: [Estabelecimento],
    })
    @ApiResponse({
        status: 404,
        description: 'Nenhum estabelecimento encontrado para o tipo de esporte',
    })
    @ApiResponse({
        status: 500,
        description: 'Erro ao buscar estabelecimentos por tipo de esporte',
    })
    async findEstabelecimentosByTipoEsporte(
        @Param('tipoEsporteId') tipoEsporteId: number,
    ): Promise<Estabelecimento[]> {
        try {
            const estabelecimentos =
                await this.estabelecimentoService.findEstabelecimentosByTipoEsporte(
                    tipoEsporteId,
                );
            if (!estabelecimentos.length) {
                throw new HttpException(
                    'Nenhum estabelecimento encontrado para o tipo de esporte',
            ...
            throw new HttpException(
                'Erro ao buscar estabelecimentos por tipo de esporte',
                HttpStatus.INTERNAL_SERVER_ERROR,
            );
        }
    }
