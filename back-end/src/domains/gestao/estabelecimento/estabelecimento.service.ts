...
export class EstabelecimentoService {
    ...
    async create(
        ...
        if (
            ...
            acomodacoes = await this.acomodacaoService.findByIdkeys(
                createEstabelecimentoDto.acomodacoesToAdd,
            );
        }
    ...
    async updateFields(
        ...
        const { nome, telefone, email, alvara, sobre } =
    ...
    async update(
        ...
        if (horariosFuncionamento && horariosFuncionamento.length > 0) {
            await this.horarioFuncionamentoService.syncHorariosFuncionamento(
                estabelecimento,
                horariosFuncionamento,
            );
        }
    ...
    async remove(idkey: number, usuario: Usuario): Promise<void> {
        ...
        if (
            ...
            estabelecimento.horariosFuncionamento.length > 0
        ) {
            await this.horarioFuncionamentoService.removeBatch(
                estabelecimento.horariosFuncionamento.map(
                    (horario) => horario.idkey,
                ),
            );
        }

        if (estabelecimento.quadras && estabelecimento.quadras.length > 0) {
            await this.quadraService.removeBatch(
                estabelecimento.quadras.map((quadra) => quadra.idkey),
            );
        }
        ...
        try {
            ...
            if (estabelecimento.endereco) {
                await this.enderecoService.remove(
                    estabelecimento.endereco.idkey,
                );
            }
        } catch (error) {
    ...
    async manageAcomodacoes(
        ...
        if (acomodacoesToAdd && acomodacoesToAdd.length > 0) {
            try {
                const acomodacoesEntities =
                    await this.acomodacaoService.findByIdkeys(acomodacoesToAdd);

                // Obter os IDs das acomodações já associadas
                const existingAcomodacaoIds = estabelecimento.acomodacoes.map(
                    (a) => a.idkey,
                );

                // Filtrar para evitar duplicações
                const novasAcomodacoes = acomodacoesEntities.filter(
                    (a) => !existingAcomodacaoIds.includes(a.idkey),
                );
        ...
        if (acomodacoesToRemove && acomodacoesToRemove.length > 0) {
            try {
                const acomodacoesEntities =
                    await this.acomodacaoService.findByIdkeys(
                        acomodacoesToRemove,
                    );
    ...
    async findEstabelecimentosByTipoEsporte(
        ...
        try {
            ...
            if (estabelecimentos.length === 0) {
                console.log(
                    `Nenhum estabelecimento encontrado para tipo de esporte com idkey ${tipoEsporteId}`,
                );
            } else {
                estabelecimentos.forEach((estabelecimento) => {
                    estabelecimento.quadras.forEach((quadra) => {
                        console.log(
                            `Quadra associada ao estabelecimento ${estabelecimento.idkey}:`,
                            quadra.nome,
                        );
                        quadra.tiposEsporte.forEach((esporte) => {
                            console.log(
                                `Tipo de Esporte na quadra ${quadra.idkey}:`,
                                esporte.descricao,
                            );
                        });
                    });
